            ...
            <div id="job_data" className="dataTables_wrapper ">
              ...
              <div className="d-sm-flex text-center justify-content-between align-items-center mt-3">
                ...
                <div className="dataTables_paginate paging_simple_numbers" id="example5_paginate">
					<Link className="paginate_button previous disabled" to="/richlist" onClick={() => activePag.current > 0 && onClick(activePag.current - 1)}>
						Previous
					</Link>
					<span>
						{paggination.map((number, i) => (
							<Link key={i} to="/richlist" className={`paginate_button  ${ activePag.current === i ? "current" : ""} `}onClick={() => onClick(i)}>
								{number}
							</Link>
						))}
					</span>
					<Link className="paginate_button next" to="/richlist" onClick={() => activePag.current + 1 < paggination.length && onClick(activePag.current + 1)}>
						Next
					</Link>
                </div>
              </div>
